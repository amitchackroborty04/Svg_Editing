...
export function ColorEditor({ showBorders, setShowBorders }: ColorEditorProps) {
  ...
  const handleColorSelect = (color: string) => {
    ...
    if (!selectedColors.some((c) => c.color === color)) {
      const newColor = { id: color, name: `Color ${color}`, color }
  ...
  return (
    <ScrollArea className="h-full">
      <div className="p-4 space-y-6">
        <h1 className="text-lg font-bold text-center">Color Editor</h1>
